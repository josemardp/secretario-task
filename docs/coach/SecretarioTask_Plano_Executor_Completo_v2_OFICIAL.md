...
## Plano Executor Completo v2 OFICIAL — Sprints, Prompts, Commit/Push e Controle de Evolução
...
> **Status desta versão:** esta é a versão revisada e oficial do plano executor. Se existir no repositório `files.zip`, `SecretarioTask_Plano_Executor_Completo.md` antigo ou `SecretarioTask_Prompt_Execucao_Autonoma_Codex.md`, esses arquivos devem ser tratados apenas como histórico/contexto. Para implementação, a fonte de verdade é este arquivo: `SecretarioTask_Plano_Executor_Completo_v2_OFICIAL.md`. O prompt de execução autônoma fica **deprecated**: não usar para rodar todos os sprints em sequência, não rodar todos os sprints em sequência. Nesta versão, commit/push na `main` e aplicação automática de migrations no Supabase remoto são autorizados **por sprint**, desde que o gate passe, o `git status` contenha apenas alterações do sprint, a migration seja forward-compatible e o `supabase db push --dry-run` passe antes do push real.
...
3. **Commit/push autorizados por sprint.** Após cumprir o escopo, atualizar docs e obter `lint`/`build` verdes, o agente deve rodar `git status`, confirmar que só há alterações do sprint, fazer `git add -A`, commitar com a mensagem indicada no prompt do sprint e executar `git push origin main`. Se houver alteração fora do escopo, conflito, dúvida de migration ou validação falha, não commitar nem dar push; parar e reportar.
4. **Aplicação automática controlada de migration no Supabase remoto.** Em sprints com migration, o agente deve criar o `.sql`, validar o escopo, rodar `supabase db push --dry-run` e, se o dry-run confirmar apenas migrations esperadas e forward-compatible, aplicar com `supabase db push --linked` antes do commit/push de código. Se a CLI não estiver autenticada/linkada, se houver SQL destrutivo não previsto, dúvida de constraint/backfill, divergência no histórico remoto ou falha no dry-run, o agente deve parar e reportar.
...
9. **Fonte única de verdade para implementação.** Quando houver divergência entre arquivos dentro de `files.zip` ou documentos antigos, este plano `v2_OFICIAL` vence. O arquivo `SecretarioTask_Prompt_Execucao_Autonoma_Codex.md` não deve ser executado; ele serve apenas como histórico do planejamento.
...
> Cada sprint abaixo traz: objetivo, justificativa, escopo incluído/excluído, arquivos prováveis, migrations, riscos, mitigação, critérios de aceite, comandos obrigatórios, atualizações de doc e commit realizado e hash. O **prompt pronto e autocontido** de cada sprint está na §5 (Prompt Sprint N), para não duplicar texto.
...
**Comandos obrigatórios.** `npm run lint`, `npm run build`. Aplicação da migration: automática controlada via Supabase CLI, com `supabase db push --dry-run` antes de `supabase db push --linked`.
...
**Riscos.** Assumir nome errado da constraint; CHECK antigo barrar eventos novos antes da migration aplicar; cliente continuar enviando `created_at`; emissão de evento bloquear conclusão; volume de eventos pesar.
**Mitigação.** Não assumir nome da constraint: inspecionar migrations/schema e, se não for possível confirmar com segurança, parar e reportar; eventos só passam a ser emitidos após a migration estar aplicada (documentar dependência); envolver emissão em try/catch não-bloqueante; manter throttle onde fizer sentido.
...
**Escopo incluído.** Reabertura (`handleStatusRevert`): ao sair de `done`, limpar `completed_at`, `resolved_at`, `resolution_type` e `started_at`, emitindo `reopened`; teto de timer em `buildCompleteUpdates` (ex.: `> 8h` → grava mas marca `actual_minutes_source='unknown'`/suspeita); criar e persistir `blocker_type` (campo sempre existente no schema, preenchimento opcional na UI) com CHECK in `waiting_third_party,no_time,priority_changed,needs_split,dependency`; adiar com motivo opcional sem bloquear captura.
**Escopo excluído.** Diagnóstico que consome esses dados (Sprint 8); UI de qualidade no dashboard (Sprint 7).
**Arquivos prováveis.** `src/components/TaskBoard.tsx` (`handleStatusRevert`, `buildCompleteUpdates`, postpone), `src/components/TimelineView.tsx` (postpone), `src/types/index.ts`, `src/lib/sync.ts`, migration `0018_blocker_type.sql`; docs.
**Migrations.** Obrigatória `0018_blocker_type.sql` para criar `blocker_type`; o campo existe no schema, mas o preenchimento pelo usuário continua opcional na UI.
**Riscos.** Limpar campos demais na reabertura; teto arbitrário cortar tempo legítimo; motivo de adiamento virar fricção.
**Mitigação.** Limpar só os quatro campos citados; teto **marca** suspeita, não descarta o valor; `blocker_type` existe sempre no schema, mas seu preenchimento é opcional e nunca bloqueia o adiar.
...
**Comandos obrigatórios.** `npm run lint`, `npm run build`. Migration `0018`: manual.
**Docs.** `DECISIONS.md` (regra de reabertura; teto; motivo opcional), `ARCHITECTURE.md` (`blocker_type` e dívida de dado), `PRD.md` (reabrir/adiar com motivo), `STATUS.md`, `SPRINT_LOG.md`, `ROADMAP.md` (Fase 2 done).
...
> **Regra de uso dos prompts:** execute um sprint por vez. O arquivo oficial é `SecretarioTask_Plano_Executor_Completo_v2_OFICIAL.md`. Se houver `files.zip`, planos antigos ou prompt autônomo na pasta, use-os apenas como histórico. Nunca execute `SecretarioTask_Prompt_Execucao_Autonoma_Codex.md` sem uma ordem humana explícita.

> Copie um prompt por vez. Cada prompt é autocontido. Todos compartilham o mesmo cabeçalho de leitura e as mesmas invariantes; o que muda é o escopo do sprint. O agente **commita e dá push na `main` ao final de cada sprint** depois de `lint`/`build` verdes e `git status` limpo quanto a alterações fora do escopo. Em sprints com migration, o agente **aplica a migration no Supabase remoto de forma controlada** antes do commit/push de código: `supabase migration list --linked`, `supabase db push --dry-run`, `supabase db push --linked`, nova conferência com `supabase migration list --linked`. Se qualquer etapa falhar ou mostrar escopo inesperado, parar e reportar.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "chore: baseline e congelamento de referência (Sprint 0)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.
...
2. Em DashboardView, nos blocos que derivam de updated_at — "Horário de pico" (peakHourData) e "Esta semana"/"hoje" (dailyData/weekTotal/todayCount) — trocar título/legenda para deixar explícito que é aproximação por data de edição enquanto completed_at não existe. Ex.: "Horário de pico (aproximado por edição — em revisão)"; "Esta semana" vira "Tarefas concluídas editadas recentemente" enquanto a fonte ainda for `status=done + updated_at`.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "fix: conter coach (updated_at não é conclusão) — Fase 0 (Sprint 1)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.
...
VALIDAÇÃO: npm run lint ; npm run build. A aplicação da migration no Supabase remoto é automática controlada: rode `supabase migration list --linked`, depois `supabase db push --dry-run`; se o dry-run listar somente a migration 0014 esperada, rode `supabase db push --linked` e depois `supabase migration list --linked` para confirmar. Se houver qualquer divergência, pare e reporte.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "feat: completed_at honesto + backfill legacy_approx — Fase 1A (Sprint 2)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.

RELATÓRIO FINAL: arquivos alterados; SQL da migration; pontos de leitura migrados para completed_at; validações; confirmação de aplicação remota da migration; riscos remanescentes; próximo sprint (Sprint 3).
```
...
4. taskFilters.ts: criar/ajustar helper isActiveTask que exclui das listas operacionais as tarefas com resolution_type IN ('cancelled','delegated','obsolete') — SEM usar deleted_at. Aplicar explicitamente em: Kanban/TaskBoard, Timeline, briefing/ranking, filtros de tarefas abertas, notificações e dashboard de fila ativa. Se algum ponto não puder ser ajustado com segurança no sprint, registrar como pendência bloqueadora antes de avançar.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "feat: resolution_type/resolved_at sem deleted_at — Fase 1B (Sprint 3)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.

RELATÓRIO FINAL: arquivos alterados; SQL; onde o helper foi aplicado; validações; confirmação de aplicação remota da migration; riscos; próximo sprint (Sprint 4).
```
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "feat: eventos confiáveis server-stamped — Fase 1C (Sprint 4)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.

RELATÓRIO FINAL: arquivos alterados; SQL; eventos emitidos e onde; confirmação de que são best-effort; validações; confirmação de aplicação remota da migration; riscos; próximo sprint (Sprint 5).
```
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "feat: origem de estimated/actual minutes — Fase 1D (Sprint 5)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.

RELATÓRIO FINAL: arquivos alterados; SQL; cada ponto de escrita e o source aplicado; validações; confirmação de aplicação remota da migration; riscos; próximo sprint (Sprint 6).
```
...
3. Adiar com motivo OPCIONAL: oferecer blocker_type sem obrigar e sem bloquear o adiar. Criar obrigatoriamente a migration 0018_blocker_type.sql (ADD blocker_type text NULL CHECK (blocker_type IN ('waiting_third_party','no_time','priority_changed','needs_split','dependency'))) e incluir em TASK_COLUMNS e types. O campo deve existir sempre; a UI apenas oferece motivo opcional, sem bloquear o adiar.

ESCOPO EXCLUÍDO: diagnóstico que consome esses dados (Sprint 8); UI de qualidade no dashboard (Sprint 7).

VALIDAÇÃO: npm run lint ; npm run build. Migration (se houver): passo manual de Josemar.

DOCS: DECISIONS.md (regra de reabertura; teto de timer; motivo opcional com blocker_type persistido); ARCHITECTURE.md (blocker_type e dívida de dado); PRD.md (reabrir; adiar com motivo); STATUS.md; SPRINT_LOG.md; ROADMAP.md (Fase 2 concluída).

MIGRATION REMOTA: se este sprint criou migration, antes do commit rode `supabase migration list --linked`, `supabase db push --dry-run`, confira o escopo, aplique com `supabase db push --linked` e confirme com `supabase migration list --linked`. Se falhar ou listar algo inesperado, pare e reporte.

COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "fix: reabertura limpa + teto de timer + adiar com motivo — Fase 2 (Sprint 6)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.

RELATÓRIO FINAL: arquivos alterados; migration 0018 criada; comportamento de reabertura e teto; validações; pendência de aplicação manual da migration; riscos; próximo sprint (Sprint 7).
```
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "feat: dashboard honesto + qualidade do dado — Fase 3 (Sprint 7)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "chore: governança da IA existente (fallback, sem diagnóstico) — Fase 5A (Sprint 9)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "feat: briefing cacheado por input_hash + prompt versionado — Fase 5B (Sprint 10)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.
...
COMMIT/PUSH OBRIGATÓRIO: após validações verdes e, quando aplicável, migration remota confirmada, rode `git status`. Se houver apenas alterações deste sprint, execute `git add -A`, `git commit -m "docs: auditoria final e fechamento da evolução Coach (Sprint 11)"` e `git push origin main`. Se houver alteração fora do escopo, falha de validação ou dúvida sobre migration/schema, NÃO commite nem dê push; pare e reporte.
...
8. **Aplicar migration no Supabase remoto** — nunca; o agente pode criar/commitar/pushar o arquivo `.sql`, mas a aplicação no banco remoto é ato operacional de Josemar. Commit e push na `main` são permitidos somente depois do gate verde e dentro do escopo do sprint.
