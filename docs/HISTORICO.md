# Histórico — SecretárioTask

> Conteúdo integral do `STATUS.md` até 08/08/2026, preservado quando o STATUS foi enxugado.
> Hotfixes, sprints, causas-raiz investigadas e validações: tudo aqui, sem corte.
...
- Isso explica retroativamente toda a novela da "remédio": as 3 séries recriadas estavam **vivas no banco** (due 01/08, 02/08 e 03/08), o app é que não as recebia. Os hotfixes de 01/08 e 03/08 corrigiram bugs reais, mas nenhum era a causa do sumiço.
...
- ~~Séries "Remédio" duplicadas~~ resolvido em 04/08: eram **4** séries vivas (01/08 21:00, 02/08 20:00, 03/08 20:00 e 04/08 20:11). Mantida a original (`b7062774`, 54 ocorrências de histórico) com a data reposicionada para 04/08 21:00; as outras três encerradas pelo modal "Esta e encerrar a recorrência". Tarefa de teste "zztestefix" também removida.
...
- Confirmar se a "remédio" recriada hoje aparece na Agenda de hoje (ou no dia certo, se ele mesmo escolheu um horário futuro na edição manual).
...
## Próximo passo recomendado
Recriar a "remédio" mais uma vez e conferir na Agenda de hoje (não só na Busca) se ela aparece corretamente.
...
## Objetivo
Corrigir bug relatado pelo Josemar: a tarefa recorrente diária "remédio" parou de gerar novas ocorrências depois de sumir duas vezes seguidas (recriada uma vez, sumiu de novo no dia seguinte).
...
- Busca por "remédio" confirmou o diagnóstico: só apareciam ocorrências antigas (já concluídas antes da série quebrar), nenhuma presente ou futura.
...
## Pendência
- A série "remédio" já quebrada precisa ser recriada manualmente mais uma vez pelo Josemar. A partir de agora, excluir uma ocorrência não mata mais a recorrência sem avisar — o app pergunta o que fazer.

## Próximo passo recomendado
Recriar a tarefa "remédio" recorrente diária no app real e confirmar visualmente (claro/escuro/mobile) que o novo modal de exclusão aparece certo, e que escolher "Só esta ocorrência" mantém a série viva no dia seguinte.
