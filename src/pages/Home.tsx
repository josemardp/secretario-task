    ...
    >
      {isDemoMode() && (
        <div className="bg-amber-500/15 border-b border-amber-500/30 text-ink px-4 py-2 text-xs flex justify-between items-center z-50 sticky top-0 backdrop-blur-sm">
          ...
          <button
            onClick={() => {
              // Limpa a query/hash de demo ANTES de exitDemoMode(): o ProtectedRoute lê
              // window.location.search de forma síncrona ao re-renderizar por causa da
              // troca de sessão, e preserva a query no redirect para /login. Sem isso,
              // o "?demo=true" sobrevivia na URL e reativava a demo instantaneamente
              // (Login.tsx reage ao mesmo parâmetro), fazendo "Sair da Demo" não sair.
