...
```ts
buildGovernedCoachAIPayload({ topTasks, allTasks, now })
...
- `contextStore` — contexto ativo e metadado de atualização local

## Stores NÃO persistidas
- `authStore` — sessão é gerenciada exclusivamente pelo Supabase Client
- chave de IA (`aiApiKey`) — não entra no `partialize` do `contextStore`
...
- idade da tarefa
- compatibilidade entre energia da tarefa e baseline fixa de energia
- contexto ativo
...
### `f_energy` (0–1) — proximidade contra baseline fixa
```text
f_energy = 1 - |energy_tarefa/10 - 5/10|
```

O valor manual de energia atual do usuário foi removido em 2026-07-01. A energia da tarefa (`task.energy`) continua influenciando o ranking, mas agora é comparada contra a baseline fixa `BASELINE_ENERGY = 5` em `src/lib/ranking.ts`.

**Justificativa:** preservar o efeito de priorização por energia da tarefa sem exigir input manual diário. A decisão completa está registrada em `DECISIONS.md` e o estado anterior em `docs/energia-removida.md`.
