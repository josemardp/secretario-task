...
## 2026-07-01 — Remoção cirúrgica da "energia atual do usuário" (UI + sync + feedback)

Decisão: remover o conceito de "energia atual do usuário" como input manual — o slider no header da Home, o campo `contextStore.currentEnergy`/`setCurrentEnergy`, a sincronização multi-device (`pushEnergyToCloud`/`setEnergyFromRemote` em `sync.ts` e `contextStore.ts`) e o campo `current_energy` enviado ao payload do Coach IA (`coachAIGuardrails.ts`, incluído no hash de cache em `coachAICache.ts`). `task.energy` (energia necessária para executar a tarefa) foi mantido intacto: editável no `TaskEditModal`, extraível pelo parser (`energia alta/media/baixa`, `eN`) e continua influenciando o ranking (`calculateTaskScore` em `ranking.ts`) — mas agora comparado contra uma baseline fixa (`BASELINE_ENERGY = 5`) em vez do valor que o usuário ajustava manualmente. Colunas `profiles.current_energy`/`active_context`/`energy_updated_at` e o trigger `trg_profiles_energy_lww` (migrations 0011/0012) permanecem intactos no banco — o app só parou de ler/escrever `current_energy`; `active_context`/`energy_updated_at` seguem sincronizados via LWW para o recurso de contexto (PM/Pessoal/Estudo/etc.), que não foi tocado nesta rodada.
Motivo: o usuário relatou baixo uso prático do slider de energia e pediu remoção cirúrgica de tudo que dependia dele — UI, sincronização e o sinal enviado para a IA — mas quis preservar o efeito de "energia da tarefa" na ordenação, para não perder o motor de priorização.
Alternativas descartadas: remover energia do score e redistribuir peso entre os outros fatores — descartada pelo usuário para preservar o comportamento de ranking já validado; dropar as colunas do banco numa migration nova — descartada para manter reversibilidade sem exigir migration de volta caso a feature seja reimplementada.
Contexto: pedido do usuário em 01/07/2026, após revisão detalhada de todas as ligações da funcionalidade de energia (UI, motor de ranking, sync multi-device e payload do Coach IA) na mesma sessão. Estado completo do "antes" documentado em `docs/energia-removida.md` para eventual reimplementação.
